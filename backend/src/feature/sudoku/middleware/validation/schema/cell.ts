...
export const cellSchema = z.object({
  ...
  candidates: z.set(
    z.number("Candidate value must be a number"),
    "Invalid candidates set",
  ),
});
...
export const candidateStringSchema = z.string().refine((val) => {
  ...
  for (const candidateStr of cells) {
    ...
    const res = z
      .string()
      .max(9, "Cannot be more than 9 digits")
      .regex(/^[1-9]+$/, "Candidates must only be values 1-9")
      .safeParse(candidateStr);
    if (!res.success) {
      return false;
    }
  }
