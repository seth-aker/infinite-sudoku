...
export class AuthenticationServiceImpl implements AuthenticationService {
  ...
  async verify(email: string, password: string): Promise<IUserDTO> {
    ...
    const storedPassword = res.password_hash
      ? Buffer.from(res.password_hash, "hex")
      : DUMMY_HASHED;
    ...
    if (!res.email_verified) {
      throw new AuthorizationError("Email not verified", {
        type: ErrorType.UNVERIFIED_EMAIL,
      });
    }
    return {
      id: res.user_id,
  ...
  async registerUser(user: z.infer<typeof registerBodySchema>) {
    ...
    if (!user.tosAcknowledged) {
      throw new AuthenticationError("Terms of Service must be acknowledged", {
  ...
  async refreshAccessToken(refreshToken: string) {
    const { token: newRefresh, userId } =
      await this.accessTokenDataSource.rotateRefreshToken(refreshToken);
  ...
  async resetPassword(token: string, newPassword: string) {
    ...
    if (!unusedToken)
      throw new NotFoundError("Invalid token", { type: "token_invalid" });
    ...
    await this.userDataSource.changePassword(
      unusedToken.userId,
      passwordHash,
      salt,
    );
