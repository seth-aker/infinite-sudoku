...
const gotoPuzzle = async (difficulty: DifficultyRating) => {
  if (
    ...
    PUZZLE_DIFFICULTY_ROUTES.includes(route.params.difficulty as string)
  ) {
  ...
  <header class="w-full p-4 flex justify-between border-b shadow">
    ...
    <div class="hidden md:flex md:flex-row">
        ...
        <NavigationMenuList>
            ...
            <NavigationMenuContent>
              ...
              <NavigationMenuLink as-child>
                <Button
                  ...
                  variant="link"
                  disabled
                  @click="() => gotoPuzzle('impossible')"
                  >Impossible (Comming Soon)</Button
                >
              </NavigationMenuLink>
            </NavigationMenuContent>
          </NavigationMenuItem>
          <NavigationMenuItem>
            <NavigationMenuLink as-child>
              ...
              <Button v-else @click="logout()" variant="link" class="mr-2 ml-0"
                >Logout</Button
              >
            </NavigationMenuLink>
          </NavigationMenuItem>
      ...
      <Toggle
        ...
        class="ml-2"
        @update:model-value="
          (val) => (val ? (colormode = 'dark') : (colormode = 'light'))
        "
      >
    ...
    <Popover class="md:hidden" v-slot="{ open }">
      ...
      <PopoverContent class="flex flex-col items-end w-64">
        <PopoverClose as-child>
          <Button @click="() => router.push({ name: 'home' })" variant="link"
            >Home</Button
          >
        </PopoverClose>
        <PopoverClose as-child>
          <Button @click="() => router.push({ name: 'about' })" variant="link"
            >About</Button
          >
        </PopoverClose>
        <Accordion collapsible>
          <AccordionItem value="new-puzzle-opts" v-slot="{ open }">
            <AccordionTrigger
              class="py-0 justify-end data-[state=open]:bg-sidebar-accent"
            >
              <Button variant="link" :data-state="open ? 'open' : 'closed'"
                >New Puzzle</Button
              >
            ...
            <AccordionContent class="flex flex-col items-end">
              <PopoverClose as-child>
                <Button @click="() => gotoPuzzle('beginner')" variant="link"
                  >Beginner</Button
                >
              </PopoverClose>
              <PopoverClose as-child>
                <Button @click="() => gotoPuzzle('easy')" variant="link"
                  >Easy</Button
                >
              </PopoverClose>
              <PopoverClose as-child>
                <Button @click="() => gotoPuzzle('medium')" variant="link"
                  >Medium</Button
                >
              </PopoverClose>
              <PopoverClose as-child>
