...
const isHighlighted = computed(() =>
  store.selectedIdx ? PEERS[store.selectedIdx].includes(cell.idx) : false,
);
const hasError = computed(() =>
  PEERS[cell.idx].some((peer) => store.cells[peer].value === cell.value),
);
...
<template>
  ...
  >
    <div
      v-if="cell.value !== 0"
      class="relative h-full w-full flex items-center justify-center text-black"
    ...
    <div v-else class="relative h-full w-full flex flex-wrap">
      <div
        v-for="(isVisable, index) in visableCandidates"
        :style="{ width: `${100 / 3}%`, height: `${100 / 3}%` }"
      >
        <span
          v-show="isVisable"
          class="font-light text-2xs flex items-center justify-center text-black"
        >
          {{ index + 1 }}
        </span>
      </div>
