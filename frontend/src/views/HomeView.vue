...
const resumePuzzle = async () => {
  ...
  if (!result.success) {
    ...
    router.push({
      name: "sudoku",
      params: { difficulty: result.body?.difficultyRating },
    });
  }
};
</script>
  ...
  <main class="w-full h-full flex items-center justify-center my-20">
    <Card class="w-[85%] md:w-100">
      ...
      <CardContent class="flex flex-col items-center justify-center">
        <Button
          v-if="userStore.isAuthenticated && userStore.currentPuzzleId"
          @click="resumePuzzle"
          class="w-52 m-1"
        >
        ...
        <Button @click="navigateToPuzzle('beginner')" class="w-52 m-1"
          >Beginner</Button
        >
        <Button @click="navigateToPuzzle('easy')" class="w-52 m-1">Easy</Button>
        <Button @click="navigateToPuzzle('medium')" class="w-52 m-1"
          >Medium</Button
        >
        <Button @click="navigateToPuzzle('hard')" class="w-52 m-1">Hard</Button>
        <Button
          disabled
          @click="router.push('/sudoku/impossible')"
          class="w-52 m-1"
